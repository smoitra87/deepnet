...
if ${clobber} || [ ! -e ${model_output_dir}/rbm1_LAST ]; then
  ...
  python ${extract_rep} ${model_output_dir}/rbm1_LAST \
    trainers/train_CD_rbm1.pbtxt hidden1 \
    ${data_output_dir}/rbm1_LAST ${gpu_mem} ${main_mem} || exit 1
fi
...
if ${clobber} || [ ! -e ${model_output_dir}/rbm2_LAST ]; then
  ...
  python ${extract_rep} ${model_output_dir}/rbm2_LAST \
    trainers/train_CD_rbm2.pbtxt hidden2 \
    ${data_output_dir}/rbm2_LAST ${gpu_mem} ${main_mem} || exit 1
fi
#
# TRAIN JOINT RB
if ${clobber} || [ ! -e ${model_output_dir}/joint_LAST ]; then
  ...
  python ${extract_rep} ${model_output_dir}/joint_LAST \
    trainers/train_CD_joint.pbtxt hidden2 \
    ${data_output_dir}/joint_LAST ${gpu_mem} ${main_mem} || exit 1
fi
