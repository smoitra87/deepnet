...
results_dir=/storage/data1/dbm/deepnet/deepnet/examples/results/exp2
...
if ${clobber} || [ ! -e ${model_output_dir}/joint_LAST ]; then
  ...
  python ${extract_rep} ${model_output_dir}/joint_BEST \
    trainers/train_CD_joint.pbtxt 'hidden1 hidden2' \
    ${data_output_dir}/joint_BEST ${gpu_mem} ${main_mem} || exit 1
fi
